...
import ru.dkolmogortsev.TaskPanelView
 ...
 */
@Singleton
class DayGridBuilderService
{
    @Inject
    private lateinit var eventPublisher: EventPublisher
    @Inject
    private lateinit var uiEntryBuilder: TimeEntryUiLineBuilderService
    ...
    inner class Builder(val date: Long, val paneWidth: DoubleExpression, val headerHeight: DoubleExpression, val taskToEntries: Map<Long, List<TimeEntry>>)
    {
        ...
        private fun buildEntries(parentPane: FlowPane, entries: List<TimeEntry>)
        {
            ...
            uiLines.forEach { gridPane ->
                ...
                (gridPane.children[0] as Label).padding = TaskPanelView.CHILD_ENTRY_INSETS
            }
        ...
        private fun setupPaneView(pane: DayGridPane)
        {
            ...
            val currentDayLabel = Label(if (isToday) "Today" else localDate.toString(TaskPanelView.pattern))
